        ...
        <div class="card shadow-sm" id="gestion">
            <div class="card-body">
                ...
                @elseif(in_array($cita->estado,['cancelada']))
                    <div class="alert alert-warning small mb-3"><i class="fa fa-ban me-1"></i> La cita fue cancelada. No se puede gestionar.</div>
                @endif
                <form action="{{ route('citas.gestion', $cita) }}" method="POST" enctype="multipart/form-data" class="small" id="form-gestion-cita">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="diagnostico" class="form-control form-control-sm" rows="2" required @disabled(in_array($cita->estado,['cancelada','concluida']))>{{ old('diagnostico', $cita->diagnostico) }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <div id="medicamentos-wrapper" class="d-grid gap-3" @class(['opacity-50'=>in_array($cita->estado,['cancelada','concluida'])])>
                            ...
                            @forelse($oldMeds as $idx => $med)
                                <div class="border rounded p-2 position-relative bg-light medicamento-item">
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6">
                                            <input type="text" name="medicamentos[{{ $idx }}][nombre_generico]" class="form-control form-control-sm" placeholder="Medicamento (nombre + presentación)" value="{{ trim(($med['nombre_generico'] ?? '') . ' ' . ($med['presentacion'] ?? '')) }}" @disabled(in_array($cita->estado,['cancelada','concluida']))>
                                        </div>
                                        <div class="col-md-3">
                                            <input type="text" name="medicamentos[{{ $idx }}][posologia]" class="form-control form-control-sm" placeholder="Posología" value="{{ $med['posologia'] ?? '' }}" @disabled(in_array($cita->estado,['cancelada','concluida']))>
                                        </div>
                                        <div class="col-md-2">
                                            <input type="text" name="medicamentos[{{ $idx }}][frecuencia]" class="form-control form-control-sm" placeholder="Frecuencia" value="{{ $med['frecuencia'] ?? '' }}" @disabled(in_array($cita->estado,['cancelada','concluida']))>
                                        </div>
                                        <div class="col-md-1">
                                            <input type="text" name="medicamentos[{{ $idx }}][duracion]" class="form-control form-control-sm" placeholder="Duración" value="{{ $med['duracion'] ?? '' }}" @disabled(in_array($cita->estado,['cancelada','concluida']))>
                                        </div>
                                    </div>
                                </div>
                            @empty
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="observaciones" class="form-control form-control-sm" rows="2" @disabled(in_array($cita->estado,['cancelada','concluida']))>{{ old('observaciones', $cita->observaciones) }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Adjuntos (imágenes / PDF)</label>
                        <input type="file" name="adjuntos[]" class="form-control form-control-sm" accept="image/*,application/pdf" multiple @disabled(in_array($cita->estado,['cancelada','concluida']))>
                    ...
                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" role="switch" id="concluirSwitch" name="concluir" value="1" @checked($cita->estado==='concluida') @disabled(in_array($cita->estado,['cancelada','concluida']))>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3" style="font-size:.85rem;">
                            ...
                            <div id="h-diagnostico" class="border rounded p-2 bg-light" style="white-space:pre-wrap;min-height:42px">—</div>
                        </div>
                        <div class="mb-3" style="font-size:.85rem;">
                            <div class="text-muted text-uppercase small">Observaciones</div>
                            <div id="h-observaciones" class="border rounded p-2 bg-light" style="white-space:pre-wrap;min-height:42px">—</div>
                        </div>
